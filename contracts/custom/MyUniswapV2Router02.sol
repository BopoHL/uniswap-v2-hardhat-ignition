...
contract MyUniswapV2Router02 is IUniswapV2Router02 {
    ...
    constructor(address _factory, address _WETH, bytes32 _pairCodeHash) public {
        require(_pairCodeHash != bytes32(0), 'MyUniswapV2Router: ZERO_PAIR_HASH');
        factory = _factory;
        WETH = _WETH;
    ...
    function _pairFor(address tokenA, address tokenB) internal view returns (address pair) {
        return MyUniswapV2Library.pairFor(factory, tokenA, tokenB, pairCodeHash);
    }
    ...
    {
        return MyUniswapV2Library.getReserves(factory, tokenA, tokenB, pairCodeHash);
    }
    ...
    {
        return MyUniswapV2Library.getAmountsOut(factory, amountIn, path, pairCodeHash);
    }
    ...
    function _getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return MyUniswapV2Library.getAmountsIn(factory, amountOut, path, pairCodeHash);
    }
    ...
    function removeLiquidity(
        ...
        (address token0,) = MyUniswapV2Library.sortTokens(tokenA, tokenB);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = MyUniswapV2Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
