...
import UniswapV2Module from "../ignition/modules/UniswapV2Module.js";
...
async function main() {
  const pairHash = ethers.keccak256(`0x${pairArtifact.bytecode}`);
  assert.equal(pairHash, CANONICAL_PAIR_HASH);
  ...
  console.log(`  Uniswap V2 Router02: ${routerAddress}`);
  console.log(`  Pair init-code hash: ${CANONICAL_PAIR_HASH} (verified)`);
  ...
  console.log("\nPool creation, liquidity, and 3 swaps verified.");
}
